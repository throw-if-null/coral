...
spine first. Rule IDs here will use the `LIB-` family, and each will carry exactly one enforcement class.
...
- **Configuration** → `[CONFIG-1]`: a library takes configuration as arguments, never from the ambient
  environment — `[CONFIG-2]` is absolute here, because the consumer owns the process.
- **Trust / security** → `[TRUST-1]` `[TRUST-2]`: validate public inputs at the API boundary; **state** the
  trust assumptions (the library trusts its caller unless documented otherwise).
