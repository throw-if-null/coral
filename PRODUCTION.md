...
The system-scale half covers channel contracts, orchestration topology, and cross-app contract testing.
It is in [`SYSTEM.md`](./SYSTEM.md), adopted by the same flag at the `system` scale. App-type specifics
are in the [appendices](./ARCHITECTURE.md#appendix-index), which are separate profiles and separate
adoptions.
...
cross-references. A few rules lead with a bolded name instead, such as `[DUP-4]` The Extraction Test and
...
large shared concept, **give that concept its own app with the dense logic inside it**, rather than
growing a shared core inside this one.

State what the split does precisely: it does not dissolve the density, it **relocates and encapsulates**
it. The new app is allowed to be internally dense. It is the kind of app `[SCOPE-2]` says this
architecture is weak for, and inside it you may use whatever model the domain requires: a rules engine, a
state machine, or a solver. What you gain is that the density is **bounded to one app behind one
published contract**, so every other app stays slice-shaped and within an agent's context. The
orchestration layer still carries no business logic, because the logic lives *inside* the dense app
rather than in the wiring. That is a system-scale rule which `SYSTEM.md` owns, so this document does not
cite it (see `[SCOPE-4]`).

The prerequisite is a contract. **If you cannot draw a published contract around the dense concept, you
do not have a split.** You have a domain this architecture does not fit. Say so and choose differently,
rather than forcing a boundary that will leak. A slice getting dense is a *split signal*, not a
refactor-into-a-shared-core signal. See `[GROW-3]`.
...
behavior, or one `repositories` package holding every capability's queries. It does *not* forbid internal
structure. **Banding within a capability is permitted** where a language or its tooling forces one
capability to span more than one package. Go's import-cycle rules and single-package code generation are
the common case. Two conditions apply: every band is still named for the capability or concern it owns,
and the dependency arrow points one way.
[`examples/go-api-slice.md`](./examples/go-api-slice.md) works through this case.
...
A crosscut is not "a helper each slice copies". That is the forbidden-bucket failure. A crosscut is one
...
external-system client, and a broker binding own no capability and carry no must-not-diverge invariant.
Calling them buckets would forbid the one shape that makes code-generated persistence work at all.
[`examples/go-api-slice.md`](./examples/go-api-slice.md) had to describe its `store` package as
"deliberately not in this table", which was the missing category made visible.

**Direction of interface ownership is the whole test**, and it is the same one `[STATE-2]` states. The
slice declares the interface, listing only the operations *this* capability uses. The adapter implements
it. The dependency arrow runs adapter → slice. Reverse the arrow and it is a `repository` layer
(`[BUCKET-1]`): the shared package defines the API, every caller consumes whatever it offers, and no
slice can be read alone.

Two consequences follow. An adapter is **never** a place to put logic. Behavior that migrates into one
has migrated *out* of a slice, and the fix is to move it back rather than to grow the adapter. One
adapter may also serve several slices, as one generated package with many ports, without becoming shared
state, because each slice still owns its own port. Name it for the infrastructure it connects to
(`store`, `s3`, `stripe`), never for a role (`[MODEL-2]`).
...
**`[BOUND-3]` `[review]` `{baseline}`** — Use the boundary form fixed by the relevant appendix, and do
not invent a new boundary kind for an app type that already has one.

**`[BOUND-4]` `[guide]` `{baseline}`** — "Continuous" or "real-time" work is **not** a new boundary kind.

Model it as one of three things: an on-demand read trigger that recomputes when asked, an event or
message handler that recomputes per event and idempotently (`[IDEM-5]`), or a scheduled or timer tick,
which is itself a trigger handled like an event handler. A genuinely long-running reconciler that owns
evolving cross-domain rules is a `[SCOPE-3]` split into its own app. That app's slices are still
triggered by reads, events, or ticks, never by an ambient loop.
...
It differs from a request in one way that matters: **it has no caller to return to**, so silent failure
is its default behavior rather than an edge case. Three requirements follow:

- Its outcome must be observable: success, failure, and what it processed (`[OBS-2]`). A job whose only
  failure signal is "the number did not go up" is not shippable.
- It must be safe under **overlapping runs**, because the next tick can fire before this one finishes
  (`[CONC-3]`). Alternatively it must decline to start while a run is in flight.
- On any platform that retries a missed or failed run, it must be idempotent (`[IDEM-5]`).

Give it a real observable contract even though no user reads it. A summary result of counts, ids touched
and a status is what makes the job testable at its entry point like every other slice (`[TEST-1]`).
...
This is statically decidable, and it is the check that enforces `[ROOT-1]`. Business logic in the root
almost always appears as an import of the thing it operates on. Pair it with a size budget appropriate
to your stack. If the root is the largest file in the app, `[ROOT-1]` is being violated whatever the
imports say.
...
The library is named here rather than only in its appendix, because it is the one case where the answer
is *nothing in this package*. Everything this section assigns to the root belongs to somebody the author
will never meet.
...
it is named. A fourth thing that layout shows, that a crosscut is rare and lives at the root, is
`[XCUT-2]`.
...
A feature package is **not** a sixth category of code. `[MODEL-1]` classifies units of code, and a
feature package is a container for them. What makes it more than a folder is that it is the **ownership
boundary for state**. Inside it, sibling slices reach the same table directly. Outside it, access goes
through a published capability (`[COMPOSE-1]`). A package holding a single slice is normal and needs no
ceremony. The boundary matters from the second operation on.
...
Do not introduce `core` as a home for code that seems central. If a module is genuinely a domain engine,
name it for the domain (`pricing`, `scoring`). A pre-existing `core` whose name already denotes one
specific bounded concept is grandfathered. Never grow it as a catch-all.

**`[BUCKET-2]` `[guide]` `{baseline}`** — Generic catch-all names destroy locality and predictability.

A forbidden bucket is a would-be crosscut with **no precise name and no injection discipline**. The cure
is not to ban all sharing. It is to make the shared thing a real crosscut
([`[XCUT-*]`](#_7-crosscut-discipline-xcut)), or to leave it duplicated
...
Injection is about testability, not ceremony. A crosscut that is **pure and stateless**, such as a
formatter, a validator, or a parse invariant, may be consumed by direct import. That *is* consuming its
published surface, and wrapping it in a container buys nothing. A crosscut that holds **configuration, a
connection, or per-trigger state** must be injected. Otherwise the slice cannot be exercised without
ambient setup, and `[CONFIG-2]` is violated.
...
If money formatting is a crosscut, there is structurally one copy and nothing to drift. Static and LLM
drift checks are the backstop for what passes this line, not a substitute for it. Do not make `[XCUT]`
the new way around the rules. `[XCUT-1]` is a gate, not a licence.
...
This is the loophole through which a layered domain model returns under a crosscut's name, so the line
is drawn explicitly. A crosscut named for the domain (`money`, `pricing`, `booking`) may own the
...
**`[DUP-1]` `[guide]` `{baseline}`** — Small duplication across slices is acceptable and often
preferred, and you should not extract merely to save lines.
...
following: enforces a business invariant, enforces a CLI, API or persistence convention that must not
diverge, provides stable infrastructure with a precise name, or materially clarifies a real domain
calculation.

**`[DUP-4]` `[review]` `{baseline}` — The Extraction Test.** Before extracting, ask three questions. Is
this a real domain or infrastructure concept? Are we protecting an invariant or enforcing a convention
that must stay consistent everywhere? Would duplication be cheaper than a permanent abstraction? If the
answers are predominantly *no*, do not extract.
...
feature package exposes no read capability the aggregator needs, that package publishes one. That is
normally a read slice of its own (`expense/list`), which is why the requirement is rarely as heavy as it
sounds. The aggregator never reaches into another package's state. The same idea across a process
boundary becomes a system concern (`[SCOPE-4]`). When the fan-in starts carrying its own growing
cross-domain rules, that is the `[GROW-3]` split signal.
...
write their own queries against it, and the first shared `queries` module is where the package becomes a
layer.

**`[STATE-2]` `[auto]` `{baseline}`** — Do not create a shared repository or data-access layer.

Connection management, pooling, and migration *execution* may be a small, precisely-named `db` crosscut.
It must not grow into a generic data-access layer (`[BUCKET-1]`).

**The direction of interface ownership is the test**, and it is what distinguishes a forbidden repository
from a legitimate adapter. If a shared package **defines** the data-access API and slices consume
whatever it offers, it is a repository layer. It accumulates every caller's needs, and no slice can be
read alone. If the **slice declares the interface it needs**, listing only the operations *this*
capability uses, and a shared package merely *implements* it, the dependency arrow runs adapter → slice
and the slice stays self-contained. That inversion is permitted. It is how code-generated persistence,
such as sqlc or an ORM's generated queries, coexists with slice ownership. See
...
cross-slice reasoning on every change, whereas local ownership keeps each slice independently
changeable.
...
It is not "shared state" and does not justify a shared layer. Persisting a derived value, such as a
cached score, is an idempotent-set effect (`[IDEM-1]`). On a redelivering platform it must be idempotent
(`[IDEM-5]`). Writing the cache is a *set* effect, so it belongs to a set- or event-named handler: a
`refresh` or `recompute` slice, or an `on_change` handler. It does **not** belong to a read-named slice.
A `show` or `GET` reads the cached value and never writes it, which keeps `[IDEM-2]` satisfied.
...
form of this rule said "exactly one owning slice", and treated two slices writing one table as a split
signal. Ordinary CRUD therefore failed the rule, and the only strictly compliant alternative was worse: a
write slice publishing a read capability for its own siblings, which is the owning slice becoming the
repository these rules exist to prevent.

**Package ownership is not permission for a shared query module.** `[STATE-1]` and `[STATE-2]` still hold
*inside* the package. Each slice writes its own queries against the shared shape. An `expense/queries`
module serving all four operations is the `repository` layer at smaller scale (`[BUCKET-1]`). This rule
moves the **ownership** boundary, not the **locality** one, and that difference is what it is for.
...
Separate the mechanism from the content. The `db` crosscut **runs** migrations. The owning package
...
(`[COMPOSE-1]`). If it genuinely shares the entity's invariant, the invariant becomes a crosscut, not the
storage (`[XCUT-5]`). **Two feature packages** writing one table is the `[GROW-3]` split signal, not a
...
Test it by deleting the cache. If a read then returns wrong data rather than slow data, the cache has
become primary storage. It has silently acquired an owner, a schema, and a durability requirement it was
never designed for. Whatever the cache holds must be derivable from the real store (`[STATE-4]`).
...
There are three workable strategies, and you must pick one explicitly:

- a **TTL**, where staleness is bounded and acceptable
- **write-through** from the slice that owns the underlying value
- **event-driven** invalidation on a change signal

A cache with no stated strategy is not shippable. It will be correct in testing and stale in production,
and the next agent has no way to know which behavior was intended. Write the choice down next to the
cache.
...
Two triggers can run at the same time. The architecture makes that safe by construction in most cases,
because a slice owns one trigger and holds nothing between triggers. The remaining cases are where the
bugs live, so the assumption is written down rather than left implicit.
...
state cannot race with a copy of itself, so the only concurrency left to reason about is the *shared*
kind: crosscuts (`[CONC-2]`) and state (`[CONC-3]`). It is statically decidable. Mutable module-level
state in a slice module is a violation. Immutable constants and lookup tables are permitted.
...
There is no third option and no default to fall back on. A connection pool, a logger, a config object,
and a pure invariant helper (`money`) are shared and must be safe for simultaneous use. A transaction
handle, the authenticated principal, and the correlation id are per-trigger. The failure mode is a
crosscut that *looks* shareable and holds one trigger's state, such as a client object caching the last
response, or a builder reused across calls. State which kind each crosscut is where it is constructed.
...
compute, then write it back. That is a lost-update race whenever two triggers interleave, and testing
almost never catches it. Either collapse it into one atomic operation, or take one of the three
strategies above and say which in the code. This is also why `[IDEM-1]` distinguishes absolute from
relative updates. A *set* to a caller-supplied value tolerates interleaving in a way an increment does
not.
...
lock for the duration of somebody else's latency. Their slow day becomes your outage. Do the external
call before the transaction or after it. If the two must be consistent, make the effect idempotent
(`[IDEM-5]`) and reconcile, rather than holding the lock.

**`[CONC-5]` `[guide]` `{baseline}`** — The architecture's concurrency default is *one trigger, one thread
of control, no shared mutable state*.

That default is what lets a reviewer, or an agent, reason about a slice in isolation. Every departure
from it removes that property for the whole app, not only for the code that uses it. Departures include a
shared mutable crosscut, a background worker, and an in-process queue. Treat introducing one as an
`[AGENT-2]` decision to flag, rather than a local implementation detail. Async work is a trigger, not an
ambient loop (`[BOUND-4]`, `[BOUND-5]`).
...
service inline is untestable without that service, and it has a hidden dependency the reviewer cannot see
in its signature. Where a flag must be evaluated per request, such as a per-user rollout, inject the
*evaluator* as a crosscut and treat its result as request-bound state (`[CONC-2]`).

**`[CONFIG-2]` `[auto]` `{baseline}`** — No slice reads the process environment, a config file, or a global
settings object directly.

This is statically decidable and worth gating. A single `os.Getenv` inside a slice defeats `[AGENT-1]`,
because the slice's world is no longer in one place. It also makes the slice untestable without ambient
setup.
...
An update that **changes state relative to its current value**, such as an increment or an append, is
...
A read-named slice module that calls an insert, update, delete, or write API fails. Deeper mutation
reached through several calls is a `[review]` concern.

**`[IDEM-3]` `[review]` `{baseline}`** — Do not make a non-idempotent operation behave idempotently
without renaming it, because names must signal behavior truthfully.
...
platform itself redelivers, as queues, webhooks, GitHub Action reruns and cron retries do, a handler
with mutating effects **must** be idempotent, via an idempotency key or a natural dedupe key, because you
do not control the retry.
...
machine contract**. The principle is universal, and the mechanism is per-appendix: a `--debug` flag to
`stderr` for CLIs, and structured logs, metrics, and trace IDs for backends.

**`[OBS-2]` `[review]` `{baseline}`** — Observability is configured globally at the root, not reinvented
per slice, and slices emit through the injected logging or tracing crosscut.
...
Assert this in behavior tests (`[TEST-4]`). It is not statically decidable, because any print call could
reach the contract channel. That is why it is `[review]` and test-covered rather than `[auto]`.

---

## 17. Public & Observable Contracts  `[CONTRACT-*]`

**`[CONTRACT-1]` `[review]` `{baseline}`** — What the outside world depends on is stable, explicit, fully
typed, and free of decoration. That surface is the machine-readable output, the HTTP API shape, and the
library public API.

**`[CONTRACT-2]` `[review]` `{baseline}`** — Changes to a public contract follow that app type's
versioning discipline: semver for libraries, API versioning for backends, and documented stability for
`--json`. The concrete rule lives in the appendix.
...
library may have a minimal trust boundary, where the invoking user or caller is trusted. "Minimal" must
still be *written down*, because an unstated trust assumption is the one an agent will silently widen.
Secrets always come from the config crosscut (`[CONFIG-4]`).
...
because behavior tests verify what the slice does and survive refactors that unit tests often do not.
...
**`[TEST-4]` `[review]` `{baseline}`** — Behavior tests validate, where relevant, the observable
contract, error rendering and codes, idempotency semantics, transactional behavior, authorization at the
boundary, and diagnostic behavior.
...
first**, for example `expense/add/` into `cli`, `behavior`, `sql` and `add_test`. Never answer file
growth with a global abstraction.

**`[GROW-3]` `[review]` `{baseline}`** — Distinguish *file* growth from *domain densification*: if
features stop being independent and all reach into one large central concept, that is a `[SCOPE-3]` split
signal.

Any one of these signals is a prompt to flag per `[AGENT-2]`:

- a single slice needs the *read* state of three or more other slices at once
- a new feature cannot be described without naming several existing capabilities
- two feature packages write the same table (`[STATE-5]`)
- one rule must change in lockstep across many slices
...
here. They are in [`ARCHITECTURE.md`](./ARCHITECTURE.md#agent-execution-contract), and they bind without
being adopted. Rules for several apps composing are in [`SYSTEM.md`](./SYSTEM.md).
...
### Semantics & effects
- `[IDEM-1]` The name signals the effect, and the implementation matches it.
...
- `[STATE-4]` The slice that computes derived state owns it. Write it from a set- or event-named handler.
- `[STATE-5]` One owning feature package per table/file/bucket, schema defined once inside it. Siblings reach it directly, and outsiders go through a published capability.
...
- `[TEST-3]` Unit tests are a scalpel. Never duplicate integration coverage, and never extract only to test.
...
1. Identify the owning capability, which is the request or trigger. → `[BOUND-1]`
2. Locate the existing feature package. If none exists, create a concrete one. → `[STRUCT-2]`
...
6. Verify the observable contract: exit code or status, channels, and `--json` or body. → `[CONTRACT-1]`
...
The generic tier model is in [`ARCHITECTURE.md`](./ARCHITECTURE.md#enforcement-drift-control). It covers
static checks, LLM review, behavior tests, and the four constraints that keep enforcement from working
against the architecture. What follows is the concrete mapping for the rules **this document** defines.
It applies to a project that has adopted the baseline, and to no other. A check that fires against a
project which never adopted the rule is a false positive by construction (`[VER-6]`).

**Tier 1, static checks (deterministic, blocking).** One per `[auto]` rule **defined in this document**.
Each appendix carries its own. Each check cites the rule ID it enforces, so a failure points back here.
Some of these ship as [`tools/coral-lint`](./tools/coral-lint/README.md), and some do not yet.
...
Reserved for `[review]` rules a static check cannot decide:

- cross-slice drift in date, money, or error formats
- "this is the Nth copy, so promote per `[XCUT-1]`?" classification calls
- `[XCUT-5]` crosscut-versus-bucket judgments
- `[STATE-5]` ownership disputes
...
own tests rather than expecting a linter to decide them.
