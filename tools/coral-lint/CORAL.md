...
**No exceptions and no extensions.** The tool passes its own gates, as the note at the top of
`coral.toml` records, and it has no local rule Coral does not already state. Both registers
are empty because there is nothing in them, not because nobody looked. An entry appears here
when a human decides one is needed (`[AGENT-4]`).
...
`--json` payload, rather than an importable API.
...
built against. A project outside this repository pins a version that has been released,
because that is the only one it can read.
