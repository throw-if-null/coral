...
/**
 * The first line of every generated contract.
 *
 * Also the marker that identifies one on disk: writeExecutionContract() discards a stale
 * contract after a failed regeneration, and this is how it tells a file this generator
 * wrote from whatever else an `--out` destination might be pointing at.
 */
...
//
// Only a file this generator wrote is discarded. An `--out` destination the caller picked
// may be anything, and deleting a file we did not produce is not a fail-closed measure, it
// is data loss — so the destination is discarded only when it opens with the heading this
// generator emits.
//
...
function destinationProblem(file, projectDir) {
  if (path.basename(file) !== ADHERENCE_FILE) return null
  const own = path.resolve(file) === path.resolve(projectDir, ADHERENCE_FILE)
  return (
    `\`${file}\` is named \`${ADHERENCE_FILE}\`, which is a project's adherence RECORD and not a` +
    ` place to put generated output.` +
...
function discardStale(file) {
  try {
    ...
    if (!fs.readFileSync(file, 'utf8').startsWith(CONTRACT_HEADING)) {
